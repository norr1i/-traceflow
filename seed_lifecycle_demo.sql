-- ============================================================
-- TraceFlow — Production-Readiness Lifecycle Demo Seed  v2
-- ============================================================
-- Corrected after type audit:
--   distribution_records.batch_id  → UUID  (live DB, not sfda_tables.sql TEXT)
--   quality_inspections.batch_id   → TEXT  (schema confirmed)
--   recalls.batch_id               → UUID  (FK capa_recall.sql)
--   capas.batch_id                 → UUID  (FK capa_recall.sql)
--   scan_events.batch_id           → UUID  (FK, multitenancy comment)
--   batch_qc_results.batch_id      → UUID  (FK to production_orders)
--   activity_logs.entity_id        → TEXT
--
-- HOW TO RUN
--   Supabase Dashboard → SQL Editor → New Query → paste → Run
--   Run ONCE. ON CONFLICT DO NOTHING prevents duplicate key errors.
--   FOR-loop inserts (scan_events) produce no duplicates (auto-UUID PK).
-- ============================================================

DO $$
DECLARE
  cid  uuid;   -- company_id (resolved from first company)
  uid  uuid;   -- user_id  (resolved from user_profiles)
  ...
  p_hyd    uuid := gen_random_uuid();   -- Hyd Cylinder (Story 2)
  p_relief uuid := gen_random_uuid();   -- Relief Valve (Story 3)
  ...
  -- Story batch UUIDs
  batch_01 uuid := gen_random_uuid();  -- Ball Valve
  batch_02 uuid := gen_random_uuid();  -- Hyd Cylinder
  batch_03 uuid := gen_random_uuid();  -- Relief Valve
...
BEGIN
  ...
  SELECT c.id,
         (SELECT up.user_id FROM user_profiles up WHERE up.company_id = c.id LIMIT 1)
  INTO   cid, uid
  FROM   companies c
  ...
  RAISE NOTICE 'Seeding to company_id = %', cid;

  -- ── 0b. Backfill NULL company_id on existing orphaned rows ───
  ...
  -- Re-read IDs in case the rows existed under different UUIDs
  ...
  INSERT INTO raw_material_lots (
    ...
    (lot_ss316, cid, m_ss316, 'LOT-2025-SS316-0891', 500.0, 'kg',
     ...
     'available',
     'Mill cert EN 10204 3.1. Heat number HN-29841. Hardness 187 HB (within spec).',
     t_base, t_base),

    (lot_cs235, cid, m_carbon, 'LOT-2025-CS235-0442', 800.0, 'kg',
     s_gulf, t_base + interval '5 days', t_base + interval '3 years',
     'consumed',
     'Consumed in Safety Relief Valve production batch and supporting orders.',
     t_base + interval '5 days', t_base + interval '5 days'),

    (lot_chrome, cid, m_chrome, 'LOT-2025-CRROD-0115', 300.0, 'kg',
     s_yanbu, t_base + interval '2 days', t_base + interval '18 months',
     'quarantine',
     'QUARANTINED: Hardness 48–51 HRC below min 58 HRC. NCR-2025-0041. Return to Yanbu in progress.',
  ...
  -- Bill of materials (production_order_id UUID)
  ...
  -- Batch QC result — batch_id UUID (FK production_orders.id)
  INSERT INTO batch_qc_results (id, batch_id, status, inspector_name, notes, inspected_at, company_id, created_at)
  VALUES (qc_01,
          batch_01,            -- UUID — no cast
          'pass', 'Khalid Al-Rashidi',
    'All 250 units inspected. Ball rotation torque 7.8 Nm (spec 7–9 Nm). '
    'Pressure test at 1.5× WP: PASS. Fire-safe seat ISO 10497: PASS. '
    'Surface finish Ra 0.8 μm. Certificate of Conformance CC-2025-0891 issued.',
    t1 + interval '9 days', cid, t1 + interval '9 days')
  ON CONFLICT DO NOTHING;

  -- Formal quality inspection — batch_id TEXT (quality_inspections schema)
  INSERT INTO quality_inspections (id, batch_id, inspector_id, inspector_name,
    inspection_date, inspection_type, status, overall_score, notes, company_id, created_at, updated_at)
  VALUES (qi_01,
          batch_01::text,      -- TEXT column — explicit cast required
          'INS-001', 'Khalid Al-Rashidi',
    (t1 + interval '9 days')::date, 'final', 'passed', 97.5,
    'Final inspection QCP-003 Rev 4. All dimensional checks ±0.05 mm. '
    'Pressure, fire-safe seat, material certs reviewed. 100% units certified. CC-2025-0891 filed.',
  ...
  -- Sales records (product_id UUID, company_id UUID)
  ...
  -- Consumer QR scans — batch_id UUID (scan_events FK to production_orders.id)
  -- 35 historical scans with 3 UA fingerprints (for Repeat Consumer Rate metric)
  FOR i IN 1..35 LOOP
  ...
  -- 12 recent scans spread across the last 7 days (feeds Scan Activity chart)
  FOR i IN 1..12 LOOP
    INSERT INTO scan_events (batch_id, scanned_at, device_type, browser, user_agent, company_id)
    VALUES (
      batch_01,                -- UUID — no cast
  ...
  -- Batch journey events (batch_id UUID FK to production_orders.id)
  ...
  VALUES
    (cid, batch_01,            -- UUID — no cast
     'raw_material.received', t1, 'warehouse@company.sa', 'raw_material',
     '{"title":"Raw Materials Received & Verified","description":"SS316 lot LOT-2025-SS316-0891 (87.5 kg) from Gulf Steel Industries. Mill cert EN 10204 3.1 verified. Cleared for production."}'::jsonb,
     t1),
    (cid, batch_01,
     'packaging.completed', t1 + interval '10 days', 'ops@company.sa', 'production_order',
     '{"title":"Packaging & Labelling Complete","description":"250 units packed in VCI-coated cartons. SFDA traceability labels applied. QR codes verified. Ready for dispatch."}'::jsonb,
     t1 + interval '10 days'),
    (cid, batch_01,
     'capa.closed', t1 + interval '11 days', 'qa@company.sa', 'production_order',
  ...
  -- Production → QC Fail → CAPA (all five stages → closed)
  -- ══════════════════════════════════════════════════════════════
  ...
  INSERT INTO batch_qc_results (id, batch_id, status, inspector_name, notes, inspected_at, company_id, created_at)
  VALUES (qc_02,
          batch_02,            -- UUID — no cast
          'fail', 'Ahmed Al-Mutairi',
    'CRITICAL FAILURE — Chrome rod hardness 48–51 HRC (spec 58–62 HRC min) across all 80 units. '
    'Bore honing tolerance ±0.05 mm actual vs ±0.02 mm spec. All 80 units QUARANTINED. '
    'NCR-2025-0041 raised. Root cause: Yanbu Precision incorrect heat treatment 850°C/2h vs 920°C/4h.',
  ...
  INSERT INTO quality_inspections (id, batch_id, inspector_id, inspector_name,
    inspection_date, inspection_type, status, overall_score, notes, company_id, created_at, updated_at)
  VALUES (qi_02,
          batch_02::text,      -- TEXT column — explicit cast required
          'INS-003', 'Ahmed Al-Mutairi',
    ...
    'Batch scrapped (destructive test, no rework possible). 8D report HYD-8D-2025-041 initiated.',
    cid, t2+interval '8 days', t2+interval '8 days')
  ON CONFLICT DO NOTHING;

  -- quality_defects.inspection_id UUID (FK to quality_inspections.id)
  ...
  VALUES
    (qi_02, 'Material Hardness Out of Spec', 'critical', 80,
     'Chrome rod 48–51 HRC across batch. Supplier applied 850°C/2h vs specified 920°C/4h heat treatment.',
     'Quarantine LOT-2025-CRROD-0115. Raise NCR-2025-0041. Return lot. Source from Gulf Steel. 100% hardness retest.',
     true, t2+interval '38 days', t2+interval '8 days'),
    (qi_02, 'Tolerance Exceeded', 'major', 80,
     'Bore honing ±0.05 mm vs ±0.02 mm spec. Correlated — wrong rod hardness caused machining tool deflection.',
     '80 units condemned and scrapped. 8D report HYD-8D-2025-041 closed.',
     true, t2+interval '38 days', t2+interval '8 days')
  ON CONFLICT DO NOTHING;

  -- capas.batch_id UUID (FK to production_orders.id)
  -- capas.inspection_id UUID (FK to quality_inspections.id)
  INSERT INTO capas (
    ...
    capa_01, cid,
    qi_02,               -- UUID FK to quality_inspections.id
    batch_02,            -- UUID FK to production_orders.id
    ...
    'LOT-2025-CRROD-0115. Furnace maintenance window deviation. CoC falsely declared correct cycle.',
    '1. Quarantine and scrap all 80 units. '
    '2. Supplier NCR-2025-0041 to Yanbu Precision. '
    '3. Return full LOT-2025-CRROD-0115 (300 kg). '
    '4. Source replacement from Gulf Steel Industries. '
    '5. 100% Rockwell hardness test before production release.',
    '1. Mandatory hardness cert for all chrome rod deliveries (Approved Supplier List update). '
    '2. In-process Rockwell check at machining stage — QCP-011 Rev 3. '
    '3. Thermal process verification added to incoming inspection checklist. '
  ...
  INSERT INTO batch_qc_results (id, batch_id, status, inspector_name, notes, inspected_at, company_id, created_at)
  VALUES (qc_03,
          batch_03,            -- UUID — no cast
          'pass', 'Mohammed Al-Harbi',
    'Final inspection per ASME UG-136. Set pressure 10.0 bar ±0.28 bar. Full-lift at 110%: PASS. '
    'Seat tightness at 90%: PASS. 150 units stamped and tagged. CoC CC-2025-0903 issued. '
    'Note: ambient-temp test only — latent high-temp spring defect undetected.',
  ...
  INSERT INTO quality_inspections (id, batch_id, inspector_id, inspector_name,
    inspection_date, inspection_type, status, overall_score, notes, company_id, created_at, updated_at)
  VALUES (qi_03,
          batch_03::text,      -- TEXT column — explicit cast required
          'INS-002', 'Mohammed Al-Harbi',
    (t3+interval '7 days')::date, 'final', 'passed', 91.0,
    'Passed — ASME UG-136. 100% cold pressure tested. Documentation complete. '
    'Note: ambient-temp only. High-temp performance assumed per spring material cert — gap identified post-recall.',
  ...
  -- recalls.batch_id UUID (FK to production_orders.id)
  -- recalls.product_id UUID (FK to products.id)
  INSERT INTO recalls (
    ...
    recall_01, cid,
    p_relief,            -- UUID FK to products.id
    batch_03,            -- UUID FK to production_orders.id
    'Voluntary Recall — VSR-05-010 Safety Relief Valves, Batch 2025-Q1-003 (150 units)',
    'Field report: 4 units failed to open at set pressure during safety test at 185°C. '
    'Post-failure: spring seat substituted (17-7PH SS actual vs Inconel 625 specified). '
    'All 150 batch units potentially affected. SFDA voluntary recall per MD-CAB-04.',
    'critical', 'closed',
    'Supplier substituted 17-7PH PH stainless for specified Inconel 625 without engineering change notice. '
    'Material cert falsely declared Inconel 625. Ambient QC pass did not detect high-temp deviation. '
    'Gap: incoming inspection lacked XRF/PMI for spring assemblies.',
    'Voluntary recall of all 150 units. Free Inconel 625 spring retrofit within 30 days. '
    'All 3 customers notified within 24h. Field teams deployed. SFDA RN-2025-VSR-003 filed. '
    'Return rate: 148/150 (98.7%). 2 units unaccounted — SFDA notified.',
  ...
  INSERT INTO capas (
    ...
    capa_02, cid,
    recall_01,           -- UUID FK to recalls.id
    qi_03,               -- UUID FK to quality_inspections.id
    batch_03,            -- UUID FK to production_orders.id
    'Spring Material Deviation — Safety Relief Valve Recall VSR-2025-Q1-003',
    'critical',
    'Undisclosed material substitution: 17-7PH SS vs Inconel 625. '
    'XRF confirmed post-failure. High-temp creep failure >160°C. Incoming QC lacked PMI/XRF.',
    '1. Full recall 150 units. 2. Free Inconel 625 spring retrofit at all 3 sites. '
    '3. Supplier SHV-Springs disqualified. 4. Warehouse XRF check — 0 further OOS. 5. SFDA RN filed 24h.',
    '1. Mandatory XRF/PMI for all Inconel/superalloy spring components before production release. '
    '2. Single-source spring supply restricted to Gulf Steel Industries. '
    '3. PMI field added to incoming inspection checklist for safety-critical components. '
    '4. Annual material audit clause added to spring supplier contracts.',
  ...
  -- ══════════════════════════════════════════════════════════════
  -- 5. ACTIVE / OPEN RECORDS (live KPI card state)
  -- ══════════════════════════════════════════════════════════════

  -- Active recall — in_progress (shows on Active Recalls KPI card)
  -- recalls.product_id UUID, no batch_id (nullable)
  INSERT INTO recalls (
    ...
    'Batch IFB-2025-Q2-007 on precautionary hold pending field sampling and thread gauge testing.',
    'medium', 'in_progress',
  ...
  -- Open CAPA: investigation stage — capas.recall_id UUID
  INSERT INTO capas (
    ...
    capa_03, cid,
    recall_02,           -- UUID FK to recalls.id
    'Thread Form Deviation Investigation — Bolt Batch IFB-2025-Q2-007',
    'major',
    'Under investigation. Metrology lab measuring thread profile on retained samples. Saudi Aramco sampling in progress.',
  ...
  -- Open CAPA: corrective action stage — no recall link, no batch FK
  INSERT INTO capas (
    id, company_id,
    ...
    'major',
    'LOT-2025-SS316-0891 received without SFDA import certificate (mandatory for KSA process industry). '
    'Mill cert present but import cert omitted.',
  ...
  -- Supporting batch QC results — batch_id UUID for all
  INSERT INTO batch_qc_results (batch_id, status, inspector_name, notes, inspected_at, company_id, created_at) VALUES
    (b04, 'pass', 'Khalid Al-Rashidi',   'Sample 50/5000: dimensional ±0.05 mm PASS. Thread GO/NO-GO PASS. Tensile 870 MPa avg (min 830). Cleared.', t_base+interval '16d', cid, t_base+interval '16d'),
    (b05, 'pass', 'Mohammed Al-Harbi',   'All 8000 nuts: pitch gauge PASS. Hardness 241 HB (spec 225–275). Plating 8 μm avg (min 5 μm). Certified.', t_base+interval '19d', cid, t_base+interval '19d'),
    (b06, 'pass', 'Sara Al-Qahtani',     'Gate valve pressure test 1.5× WP: PASS. Seat leakage nil. Handwheel 32 Nm (spec ≤40 Nm). 120 units certified.', t_base+interval '27d', cid, t_base+interval '27d'),
    (b07, 'pass', 'Omar Al-Shamrani',    'MCCB trip test 10× rated: 28 ms avg (spec ≤30 ms). Insulation >500 MΩ. All 200 pass IEC 60947-2.', t_base+interval '31d', cid, t_base+interval '31d'),
    (b08, 'fail', 'Ahmed Al-Mutairi',    'VFD EMC pre-compliance FAIL: emissions 12 dB above EN 55011 Class A at 150 kHz. PCB filter stage error. Quarantined.', t_base+interval '41d', cid, t_base+interval '41d'),
    (b09, 'pass', 'Khalid Al-Rashidi',   'Gear pump: volumetric efficiency 95.2% (spec ≥93%). Pressure 375 bar 1.5× WP PASS. Noise 68 dB(A) (spec ≤72). Cleared.', t_base+interval '47d', cid, t_base+interval '47d'),
    (b10, 'hold', 'Sara Al-Qahtani',     'Flange facing Ra 3.6 μm on 12/500 (spec ≤3.2 μm). QC hold. Rework 12 units authorised. Re-inspect within 72h.', t_base+interval '53d', cid, t_base+interval '53d'),
    (b11, 'pass', 'Abdullah Al-Zahrani', 'Helmet impact max 3.5 kN (spec ≤5 kN EN 397). Penetration PASS. Chin strap PASS. 1200 units certified.', t_base+interval '57d', cid, t_base+interval '57d'),
    -- Recent results (last 7 days) for Weekly Inspections KPI
    ...
    (b13, 'hold', 'Ahmed Al-Mutairi',    'In-process: 3/20 gate valve seats show minor pitting. Hold for rework assessment. Not a safety issue.', now()-interval '1d', cid, now()-interval '1d')
  ON CONFLICT DO NOTHING;

  -- Supporting quality inspections — batch_id TEXT for all (quality_inspections schema)
  ...
  -- Supporting scan events — batch_id UUID for all
  FOR i IN 1..8 LOOP
    INSERT INTO scan_events (batch_id, scanned_at, device_type, browser, user_agent, company_id)
  ...
  -- ══════════════════════════════════════════════════════════════
  -- 7. ACTIVITY LOGS
  -- entity_id is TEXT (activity_logs schema) — ::text casts correct
  -- ══════════════════════════════════════════════════════════════

  INSERT INTO activity_logs (company_id, actor_user_id, actor_email, action_type, entity_type, entity_id, message, created_at)
  VALUES
    (cid, uid, 'ops@company.sa',       'production_order.created', 'production_order', batch_01::text, 'Production order created: 250 × Ball Valve 2in 316 SS (VBC-2IN-316).', t1),
    (cid, uid, 'qa@company.sa',        'qc_result.added',          'production_order', batch_01::text, 'QC PASS: Ball Valve batch. Certificate CC-2025-0891 issued. Cleared for shipment.', t1+interval '9d'),
    ...
    (cid, uid, 'qa@company.sa',        'capa.status_changed',      'capa',             capa_01::text,  'CAPA advanced to Corrective Action. Supplier NCR issued. Replacement lot sourced.', t2+interval '18d'),
    (cid, uid, 'qa@company.sa',        'capa.closed',              'capa',             capa_01::text,  'CAPA closed. Verification complete. Replacement lot passed 100% hardness test.', t2+interval '42d'),
    ...
    (cid, uid, 'qa@company.sa',        'qc_result.added',          'production_order', batch_03::text, 'QC PASS: Relief Valve batch. 150 units certified at ambient temp. CC-2025-0903.', t3+interval '7d'),
    (cid, uid, 'qa@company.sa',        'recall.created',           'recall',           recall_01::text,'Voluntary recall initiated: VSR-05-010 batch 2025-Q1-003. Spring material deviation. SFDA RN filed.', t3+interval '25d'),
    (cid, uid, 'qa@company.sa',        'capa.created',             'capa',             capa_02::text,  'CAPA opened: Spring material deviation — linked to voluntary recall VSR-2025-Q1-003.', t3+interval '25d'),
    (cid, uid, 'qa@company.sa',        'capa.closed',              'capa',             capa_02::text,  'CAPA closed. All 3 customer sites retrofitted. SFDA notified of closure.', t3+interval '62d'),
    (cid, uid, 'qa@company.sa',        'recall.created',           'recall',           recall_02::text,'Precautionary hold: Bolt batch IFB-2025-Q2-007. Saudi Aramco thread deviation under investigation.', now()-interval '8d'),
    (cid, uid, 'warehouse@company.sa', 'qc_result.added',          'production_order', b13::text,      'QC HOLD: Gate Valve in-process. 3/20 sample seats show pitting. Rework assessment initiated.', now()-interval '1d'),
    (cid, uid, 'qa@company.sa',        'capa.created',             'capa',             capa_04::text,  'CAPA opened: Missing SFDA import cert for LOT-SS316-0891. Corrective action in progress.', now()-interval '14d')
  ON CONFLICT DO NOTHING;

  RAISE NOTICE '=== Seed complete ===';
  RAISE NOTICE 'Story 1 — Ball Valve (complete lifecycle):  batch_01 = %', batch_01;
  RAISE NOTICE 'Story 2 — Hydraulic Cylinder (CAPA):       batch_02 = %', batch_02;
  ...
  RAISE NOTICE 'Product Journey URL: /trace/<batch_01 UUID shown above>';
