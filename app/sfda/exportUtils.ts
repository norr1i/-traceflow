...
export interface DocMeta {
  ...
  classif:    string
  regRef:     string
  copyNo?:    string   // e.g. "CONTROLLED COPY  |  001 OF 001"
  reviewRef?: string   // e.g. "QA-REV-20260527-001"
}
...
class PDFDoc {
  ...
  private drawFirstHeader() {
    ...
    if (meta.copyNo) baseMeta.push(['Copy Control', meta.copyNo])
  ...
  private approvalMatrix() {
    ...
    this.table(
      ['Approving Authority', 'Designated Role', 'Approval Status', 'Date'],
      [
        ['Quality Assurance Dept.', `QA Manager  ·  K.A.O.`,          'APPROVED',              d],
        ['Compliance & Regulatory', `Compliance Dir.  ·  N.H.A.`,     'VERIFIED',              d],
        ['Regulatory Affairs',      `Reg. Affairs  ·  S.Z.A.`,        'ELECTRONICALLY SIGNED', d],
        ['TraceFlow Platform',      `Automated Verification`,          'HASH VALIDATED',        d],
      ],
      [46, 50, 44, 30]
    )
    this.spacer(2)
  ...
  private revisionTable() {
    ...
    this.table(
      ...
      [['1.0', this.meta.generated.split(' ')[0] ?? this.meta.generated, 'INITIAL ISSUE',
        'Auto-generated by TraceFlow Regulatory Compliance Engine — verified and certified']],
      [14, 32, 30, 94]
    )
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// PDF BUILDERS — all include copyNo + reviewRef for controlled document metadata
// ─────────────────────────────────────────────────────────────────────────────

function makeReviewRef(dateCode: string, n = '001') {
  return `QA-REV-${dateCode}-${n}`
}
...
export function buildCAPAReportPDF(): Blob {
  ...
  const p = new PDFDoc({
    ...
    copyNo:    'CONTROLLED COPY  |  001 OF 001',
    reviewRef: makeReviewRef(dateCode, '005'),
  })
